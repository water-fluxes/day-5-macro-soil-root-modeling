...
getSUF <- function(table_data = NULL,
  ...
  Psi_sr_homogeneous <- -1      # Homogeneous soil-root potential
...
plot_conductivities <- function(conductivities){
  print(conductivities%>%ggplot()+
    geom_line(aes(x,y, colour = type))+
    facet_wrap(type~order_id, scale = "free")+
